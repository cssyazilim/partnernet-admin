...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-4 lg:p-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select id="dealerFilter" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="sm:col-span-2 lg:col-span-1">
                        ...
                        <div class="relative">
                            <input type="text" id="searchInput" placeholder="Müşteri adı veya telefon..." class="w-full border border-gray-300 rounded-lg px-3 py-2 pl-10 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <!-- Pagination -->
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0">
                ...
                <div class="flex space-x-1 sm:space-x-2 order-1 sm:order-2">
                    <button id="prevPage" class="px-2 sm:px-3 py-2 border border-gray-300 rounded-lg text-xs sm:text-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                        <span class="hidden sm:inline">Önceki</span>
                        <i class="fas fa-chevron-left sm:hidden"></i>
                    </button>
                    <button class="px-2 sm:px-3 py-2 bg-blue-600 text-white rounded-lg text-xs sm:text-sm">1</button>
                    <button class="px-2 sm:px-3 py-2 border border-gray-300 rounded-lg text-xs sm:text-sm hover:bg-gray-50 hidden sm:block">2</button>
                    <button class="px-2 sm:px-3 py-2 border border-gray-300 rounded-lg text-xs sm:text-sm hover:bg-gray-50 hidden sm:block">3</button>
                    <button id="nextPage" class="px-2 sm:px-3 py-2 border border-gray-300 rounded-lg text-xs sm:text-sm hover:bg-gray-50">
                    ...
                    <div class="bg-gray-50 px-4 sm:px-6 py-4 border-b border-gray-200">
                        <div class="flex items-start sm:items-center justify-between">
                            <div class="flex items-start sm:items-center flex-1 min-w-0">
                                <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm sm:text-lg flex-shrink-0">
                                    ${dealer.name.charAt(0)}
                                </div>
                                <div class="ml-3 sm:ml-4 flex-1 min-w-0">
                                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 truncate">${dealer.name}</h3>
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-xs sm:text-sm text-gray-600">
                                        <span class="flex items-center"><i class="fas fa-map-marker-alt mr-1"></i>${dealer.location}</span>
                                        <span class="flex items-center sm:hidden"><i class="fas fa-phone mr-1"></i>${dealer.phone}</span>
                                        <span class="hidden sm:flex items-center"><i class="fas fa-phone mr-1"></i>${dealer.phone}</span>
                                        <span class="flex items-center"><i class="fas fa-users mr-1"></i>${dealer.customers.length} Müşteri</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex flex-col sm:flex-row items-end sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 ml-2">
                                ...
                                <button class="text-gray-400 hover:text-gray-600 p-1" onclick="toggleDealer('${dealer.id}')">
                                    ...
                                    <div class="p-3 hover:bg-gray-50">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center space-x-3 flex-1 min-w-0">
                                                <div class="h-8 w-8 rounded-full bg-indigo-500 flex items-center justify-center text-white font-medium text-sm flex-shrink-0">
                                                    ${customer.name.charAt(0)}
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <div class="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600">
                                                        ...
                                                        <span>Kayıt: ${customer.registerDate}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="flex space-x-2 ml-2">
                                                <button class="text-blue-600 hover:text-blue-900 p-1" onclick="viewCustomer(${customer.id})">
                                                    <i class="fas fa-eye text-sm"></i>
                                                </button>
                                                <button class="text-green-600 hover:text-green-900 p-1" onclick="editCustomer(${customer.id})">
                                                    <i class="fas fa-edit text-sm"></i>
                                                </button>
                                                <button class="text-red-600 hover:text-red-900 p-1" onclick="deleteCustomer(${customer.id}, '${dealer.id}')">
...
<script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'9818d85205cb88e2',t:'MTc1ODI4MjQxMi4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
</html>
